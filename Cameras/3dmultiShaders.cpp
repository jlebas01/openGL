...
void key_callback(GLFWwindow * window, int key, int sancode, int action, int mods){
    //TrackBallCamera *camera = static_cast<TrackBallCamera>(glfwGetWindowUserPointer(camera));
...
int main(int argc, char** argv) {
    ...
    auto window_callback_context = [](GLFWwindow * w, int key, int sancode, int action, int mods){
        ...
        switch(key){
            case 87 : //Z
            {camera_ptr -> camera.rotateUp(0.05f);
                break;
            }
            case 83 : //S
            {camera_ptr -> camera.rotateUp(-0.05f);
            ...
            case 65 : //Q
            {camera_ptr -> camera.rotateLeft(0.01f);
                break;
            }
            case 68 : //D
            {camera_ptr -> camera.rotateLeft(-0.01f);
            ...
            case 265 : //pageUp
            {camera_ptr -> camera.moveFront(0.2);
                break;
            }
            case 264 : //pageDown
            {camera_ptr -> camera.moveFront(-0.2);
    ...
    //TrackBallCamera camera;
    objects.camera = TrackBallCamera();
    ...
    glm::mat4 earthMVMatrix = glm::mat4(1.f);
    ...
    while (!glfwWindowShouldClose(windowManager)) {
        ...
        glm::mat4 cameraMatrix = objects.camera.getViewMatrix();
       
        
        earthMVMatrix = globalMVMatrix * cameraMatrix;
        
        earthMVMatrix = glm::rotate(earthMVMatrix, (float)glfwGetTime(), glm::vec3(0, 1, 0));
        ...
        for(int i = 0; i < 8 ; i ++){
            /*-----------------------S2--------------------------*/
            //bindTexture(indexTex, img_texMoon);
            moonProgram.m_Program.use();
            MVMatrix = globalMVMatrix * cameraMatrix;
            MVMatrix = glm::rotate(MVMatrix, (float)glfwGetTime()/4, rotations.at(i)); // Translation * Rotation
            MVMatrix = glm::translate(MVMatrix, positions.at(i)); // Translation * Rotation * Translation
            MVMatrix = glm::scale(MVMatrix, glm::vec3(0.2, 0.2, 0.2)); // Translation * Rotation * Translation * Scale
            NormalMatrix = glm::transpose(glm::inverse(MVMatrix));
            //MVMatrix = MVMatrix * cameraMatrix;
            ...
            bindTexture2(tex_names[0], moonProgram.img_texMoon, GL_TEXTURE0);
            /*-----------------------S2--------------------------*/
